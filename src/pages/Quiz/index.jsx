...
export default function Quiz() {
  ...
  const TIMOUT = 3000;
  if (ID === "Quiz" || ID === "") window.location.href = "/dashboard";
  const getData = async () => {
    try {
          ...
          >
            ...
            {Exam?.questions[activeStep] && (
              <Typography variant="h5">
                {Exam?.questions[activeStep]?.questionText}
              </Typography>
            )}
            <FormControl>
              <RadioGroup>
                ...
                {Exam?.questions[activeStep] && (
                  <>
                    <FormControlLabel
                      value={Exam?.questions[activeStep]?.optionOne}
                      control={<Radio />}
                      label={Exam?.questions[activeStep]?.optionOne}
                    />
                    <FormControlLabel
                      value={Exam?.questions[activeStep]?.optionTwo}
                      control={<Radio />}
                      label={Exam?.questions[activeStep]?.optionTwo}
                    />
                    <FormControlLabel
                      value={Exam?.questions[activeStep]?.optionThree}
                      control={<Radio />}
                      label={Exam?.questions[activeStep]?.optionThree}
                    />
                    <FormControlLabel
                      value={Exam?.questions[activeStep]?.optionFour}
                      control={<Radio />}
                      label={Exam?.questions[activeStep]?.optionFour}
                    />
                  </>
                )}
              </RadioGroup>
